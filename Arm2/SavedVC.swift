...
class SavedVC: UIViewController,UINavigationControllerDelegate,  UICollectionViewDataSource, UICollectionViewDelegate {
...
//    }
    ...
    func getAllCases() -> [String]{
        ...
        //        print(mainPath)
                let documentDirecortPath = mainPath + "/Saved"
                let d = URL(string: documentDirecortPath)!
                var fileURLs : [URL] = []
                let fileManager = FileManager.default
                let p = Bundle.main.resourcePath! + "/Saved"
...
//            cell.cV.document = pdfDocument
        ...
        if titleAr.count > 0{
            let path = Bundle.main.path(forResource: "/Saved/"+s1, ofType: "pdf") // read resource name: pdf.pdf
